#ifndef VTR_SPARSE_MATRIX_H
#define VTR_SPARSE_MATRIX_H
#include <vector>
#include <iterator>

namespace vtr {

    //A 2 dimensional matrix with rows indexed by Index0,
    //and each row of variable length (indexed by Index1)
    //
    //Example:
    //
    //      std::vector<int> row_sizes = {1, 5, 3, 10};
    //      FlatSparseMatrix<float> matrix(row_sizes);
    //
    ...
    class FlatSparseMatrix {
        public: //Forward declarations
            template<typename U> class ProxyRow;

        public: //Lifetime
            FlatSparseMatrix() = default;
            ...
            FlatSparseMatrix(size_t nrows, Callback& row_length_callback, T default_value=T())
                : FlatSparseMatrix(RowLengthIterator<Callback>(0, row_length_callback), 
                                   RowLengthIterator<Callback>(nrows, row_length_callback),
                                   default_value) {}

            //Constructs matrix from a container of row lengths
            template<class Container>
            FlatSparseMatrix(Container container, T default_value=T())
                : FlatSparseMatrix(std::begin(container), std::end(container), default_value) {}
            ...
            template<class Iter>
            FlatSparseMatrix(Iter row_size_first, Iter row_size_last, T default_value=T()) {
        ...
        public: //Accessors
            ...
            ProxyRow<T> operator[](Index0 i) {
                int idx = size_t(i);
                return ProxyRow<T>(&data_[first_elem_[idx]],
                                   &data_[first_elem_[idx+1]]);
            }

            ProxyRow<const T> operator[](Index0 i) const {
                int idx = size_t(i);
                return ProxyRow<const T>(&data_[first_elem_[idx]],
                                         &data_[first_elem_[idx+1]]);
            }
        ...
        private:
            //Iterator for constructing FlatSparseMatrix which uses a callback to determine
            //row lengths.
            template<class Callback>
